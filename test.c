...
void handler_test_msg(unsigned long src, unsigned long dst,
		...
		void* msg, size_t size)
{
	...
	if(total_msgs % 400000 == 0) {
		printf(
			"dequeue. per msgs \033[1;31m%lf ns\033[m, msgs (total %ld,  err %ld).\n"\
...
pthread_t new_enqueue_task(unsigned long moduleId,
			unsigned long dstModuleId, char *moduleName,
